...
export default function ModernCreatorProfile({
  ...
  const metrics = {
    ...
    projects: profile.metrics?.projects || profile.projects?.length || profile.completedProjects || 0,
    rating: profile.ratings?.average || profile.rating || 0,
    ratingCount: profile.ratings?.count || profile.ratingCount || 0
  };
            ...
            <div className="bg-white rounded-xl shadow-sm p-8">
              ...
              {(profile.reviews && profile.reviews.length > 0) ? (
                <div className="space-y-6">
                  {profile.reviews.slice(0, 3).map((review: any, index: number) => (
                    <div key={index} className="border-b border-gray-100 pb-6 last:border-0">
                      ...
                      <p className="text-gray-600 text-sm">
                        {typeof review.content === 'string' ? review.content : "No comment provided."}
                      </p>
                    </div>
                  ))}
                </div>

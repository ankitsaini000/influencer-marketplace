...
const PublishedCreators: React.FC<PublishedCreatorsProps> = ({ 
  ...
  const loadMoreCreators = async () => {
    ...
    try {
      ...
      if (!result || !result.creators || !Array.isArray(result.creators)) {
        ...
        setCreators(prevCreators => [...prevCreators, ...result.creators]);
        setHasMore(result.pagination?.hasMore || false);
        setPage(nextPage);
      }
    } catch (err) {
  ...
  const mapCreatorToCardProps = (creator: any) => {
    ...
    // Map social media URLs ensuring they exist
    ...
    const category = creator.professionalInfo?.category || creator.category || 'Creator';
    ...
    const description = creator.descriptionFaq?.briefDescription || 
                        ...
                        creator.description || 
                        creator.bio ||
                        `Creator specializing in ${category}`;
    ...
    const reviewCount = creator.reviews || 
                        (creator.metrics && creator.metrics.ratings ? creator.metrics.ratings.count : 0) || 
                        0;
